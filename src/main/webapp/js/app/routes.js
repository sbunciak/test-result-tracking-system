require([ "app/models/Product",
          ...
          "app/views/ProductVersionCloneView",
          ...
          "app/views/HomeView", 
          "app/views/TestReportView" ],

function(Product, ProductVersion, ProductBuild, Axis, AxisValue, AxisConfig,
		TestPlan, TestCase, TestRun, TestRunCase, Products, ProductVersions,
		ProductBuilds, Axiss, AxisValues, AxisConfigs, TestPlans, TestCases,
		TestRuns, TestRunCases, ProductListView, ProductCreateUpdateView,
		ProductVersionListView, ProductVersionCloneView, ProductVersionCreateUpdateView,
		ProductBuildListView, ProductBuildCreateUpdateView, AxisListView,
		AxisCreateUpdateView, AxisValueListView, AxisValueCreateUpdateView,
		AxisConfigListView, AxisConfigCreateView, AxisConfigUpdateView,
		TestPlanListView, TestPlanCreateUpdateView, TestCaseListView,
		TestCaseCreateUpdateView, TestRunListView, TestRunCreateUpdateView,
		TestRunCaseListView, TestRunCaseUpdateView, HomeView, TestReportView) {
	...
	var AppRouter = Backbone.Router.extend({
		routes : {
			...
			// Test report
			"test_report" : "generateTestReport", // generate test report
		},
		...
		generateTestReport : function() {
			...
			testRuns.url = "rest/products/" + prd_id + "/versions/" + ver_id
					+ "/builds/" + bld_id  + "/runs";

			currentView.close();
			currentView = new TestReportView({
				el : $("#page_loader"),
				model : testRuns
			});

			testRuns.fetch({success:function() {
				currentView.render();
			}});
		},
		...
		listTestRunCases : function() {
			...
			testRunCases.url = "rest/products/" + prd_id + "/versions/"
					+ ver_id + "/builds/" + bld_id + "/testplans/" + tpl_id
					+ "/runs/" + run_id + "/cases";
		...
		editTestRunCase : function(case_id) {
			...
			testRunCase.urlRoot = "rest/products/" + prd_id + "/versions/"
					+ ver_id + "/builds/" + bld_id + "/testplans/" + tpl_id
					+ "/runs/" + run_id + "/cases";
		...
		createTestRun : function() {
			...
			testRun.urlRoot = "rest/products/" + prd_id + "/versions/" + ver_id
		...
		editTestCase : function(case_id) {
			...
			testCase.urlRoot = "rest/products/" + prd_id + "/versions/"
					+ ver_id + "/testplans/" + tpl_id + "/cases";
		...
		listTestCases : function() {
			...
			testCases.url = "rest/products/" + prd_id + "/versions/" + ver_id
		...
		editTestPlan : function(plan_id) {
			...
			testPlan.urlRoot = "rest/products/" + prd_id + "/versions/"
					+ ver_id + "/testplans";
		...
		listTestPlans : function() {
			...
			testPlans.url = "rest/products/" + prd_id + "/versions/" + ver_id
					+ "/testplans";
		...
		createAxisConfig : function() {
			...
			axisConfigs.url = 'rest/axis/' + aid + '/version/' + ver_id
					+ '/configurations';
		...
		editAxisConfig : function(config_id) {
			...
			axisConfig.urlRoot = 'rest/axis/' + aid + '/version/' + ver_id
					+ '/configurations';
		...
		editProductBuild : function(build_id) {
			...
			productBuild.urlRoot = 'rest/products/' + pid + '/versions/' + vid
					+ '/builds';
		...
		listProductBuilds : function() {
			...
			productBuilds.url = 'rest/products/' + pid + '/versions/' + vid
					+ '/builds';
		...
		cloneProductVersion : function(version_id) {
			...
			productVersion.fetch({
				success : function() {
					var clone = productVersion.cloneWithoutId();
					clone.urlRoot = "rest/products/" + pid + "/versions/";
					
					currentView.close();
					currentView = new ProductVersionCloneView({
						el : $("#page_loader"),
						model : clone
					});
					
					currentView.cloning_id = version_id;
					currentView.render();
				}
			});
		},
		...
		cloneProduct : function(product_id) {
			...
			product.fetch({
				success : function() {
					var clone = product.cloneWithoutId();
